...
self-consistency cannot. It does not demonstrate the full algebraic
structure of quantum mechanics: the Born rule, the composition rule for
subsystems, and interference in measurement outcomes require a larger
model ($N \geq 8$) where these properties can be tested independently.
The toy model confirms one link of the co-emergence chain in a minimal
setting; the chain as a whole remains Conjecture.
\end{remark}
...
probabilities---not merely classical stochastic correlations. The finite
toy model (Remark~\ref{rem:toy_model}) demonstrates that Lorentzian
self-consistency produces complex off-diagonal density matrix elements at
$N = 4$, confirming that the mechanism of Section~\ref{sec:phase} is
operative in the minimal case. But complex off-diagonal elements are
necessary, not sufficient: the decisive question is whether larger models
($N = 8$, $16$) produce the full algebraic structure of quantum
mechanics---the Born rule, the composition rule for subsystems
($\rho_{R_1} = \mathrm{Tr}_{R_2}\,\rho_R$), and interference in
measurement outcomes. If larger models show only classical correlations
with a complex sheen---off-diagonal phases that do not compose correctly
or produce the right measurement statistics---then the framework
reorganizes quantum mechanics but does not derive it, and the paper
should say so.
...
(Remark~\ref{rem:toy_model}) finds no such structure at $N = 4$; a
construction at any $N$ would kill the thesis.
...
at $N = 4$, but this could be a finite-size artifact. If Lorentzian
self-consistency produces only classical correlations in the $N \to
\infty$ limit, the mechanism of Section~\ref{sec:phase} fails.
...
rule. Complex off-diagonal elements are necessary but not sufficient for
quantum mechanics; the composition and measurement structure must also
emerge.
