...
weakened: the conformal trace anomaly generates nonzero curvature, which
induces effective mass for non-conformally coupled fields, closing the
co-emergence loop without bare mass as input.
A finite toy model confirms the mechanism: at $N = 8$ and $16$,
Lorentzian self-consistency produces subsystem density matrices
satisfying the Born rule, correct subsystem composition, and destructive
interference---all absent in the Riemannian case. The quantum fraction
of subsystem coherence (the ratio of imaginary to real off-diagonal
density matrix norms) plateaus when subsystem dimension scales with $N$,
confirming that the quantum signal survives scaling.
...
followed by the self-consistent mass generation mechanism that weakens
the mass assumption. Section~\ref{sec:hierarchy} develops the four-level
hierarchy.
...
%----------------------------------------------------------------------
\subsection{The phase structure of $\mu^*$}
\label{sec:phase}
%----------------------------------------------------------------------

The co-emergence thesis requires that marginalizing $\mu^*$ over the
...
on a structural property of $\mu^*$ that the paper has so far left
unspecified: is $\mu^*$ real-valued or complex-valued?
...
\begin{remark}[What emergence means here]
\label{rem:emergence}
...
The critical question for the toy model (Section~\ref{sec:open}) is
therefore two-part: (1) does the self-consistency condition
$\mathcal{F}(\mu^*) = \mu^*$ naturally produce complex-valued fixed
points even when the input space is purely topological, or must the
phases be imposed by hand? And (2) if $\mu^*$ is complex, does the
marginal over a bipartition have the algebraic structure of a density
matrix? A positive answer to both would demonstrate that self-consistency
generates the phase structure that marginalization then organizes into
local quantum mechanics. A negative answer to (1) would mean the
framework reorganizes quantum mechanics but does not derive it, and this
paper should be read accordingly.

\begin{remark}[Finite toy model]
\label{rem:toy_model}
A finite truncation ($N = 4$, $\mathrm{Sm}(\mathcal{M}) = S_R \times
S_C$ with $|S_R| = |S_C| = 2$) with an explicit self-consistency map
$\mathcal{F}$ partially answers both questions. In the minimal model:
\begin{itemize}
...
Lorentzian/Riemannian boundary, confirming the mechanism of this
subsection.
\end{itemize}
...
subsystem. Destructive interference---measurement bases where
$p_{\mathrm{quantum}}(k) < p_{\mathrm{incoherent}}(k)$---is present
for all Lorentzian fixed points and absent for all Riemannian ones.
\item \textbf{$N = 16$ ($2 \times 2 \times 2 \times 2$):} A
three-level composition chain
$\rho_{ABC} \to \rho_{AB} \to \rho_A = \rho_A^{(\mathrm{direct})}$
passes for all tested paths. Born rule and destructive interference
confirmed across five random seeds for the external field~$h$.
\item \textbf{Scaling ($N = 4$--$64$):} When subsystem dimension is
held at $2$ while $N$ grows (the original $\mathrm{dims} =
(2,2,\ldots,2)$ setup), the imaginary coherence
$|\mathrm{Im}\,\rho_{01}|$ decays as $\sim N^{-0.6}$. This is a
projection artifact: a fixed-dimensional window onto a growing space
geometrically dilutes all off-diagonal structure. When instead the
subsystem dimension scales with $N$---tracing out a fixed-dimensional
environment ($\mathrm{dims} = (d_{\mathrm{sub}}, d_{\mathrm{env}})$
with $d_{\mathrm{env}}$ fixed and $d_{\mathrm{sub}}$ growing)---the
ratio of imaginary to real off-diagonal Frobenius norms
$\|\mathrm{Im}\,\rho^{\mathrm{od}}\|_F /
\|\mathrm{Re}\,\rho^{\mathrm{od}}\|_F$ \textbf{plateaus}: at $\approx
0.27$ for $d_{\mathrm{env}} = 2$ and $\approx 0.21$ for
$d_{\mathrm{env}} = 3$, stable from $d_{\mathrm{sub}} = 4$ through
$32$ with contracting variance. The entropy ratio
$S_{\mathrm{Lor}}/S_{\mathrm{Riem}} \approx 1.67$ is similarly stable
across both scaling regimes.
\end{itemize}
...
(composition, Born rule, interference) and provides numerical evidence
that the mechanism survives scaling; the co-emergence chain as a whole
remains Conjecture.
\end{remark}
...
(Remark~\ref{rem:toy_model}) confirms at $N = 8$ and $16$ that
...
and destructive interference---all absent in the Riemannian case.
A scaling study shows that the imaginary coherence of a fixed
dim-$2$ subsystem decays as $\sim N^{-0.6}$, but this is a projection
artifact: when subsystem dimension scales with~$N$ (fixed environment
dimension), the ratio of imaginary to real off-diagonal norms plateaus
at $\approx 0.27$ ($d_{\mathrm{env}} = 2$) and $\approx 0.21$
($d_{\mathrm{env}} = 3$), confirming that the quantum fraction of
coherence survives scaling. However, the Born rule
...
(Remark~\ref{rem:toy_model}) finds no such structure at $N = 4$ or at
...
at $N = 4$. When a dim-$2$ subsystem is traced from an $N$-dimensional
space, $|\mathrm{Im}\,\rho_{01}|$ decays as $\sim N^{-0.6}$, but
this is a projection artifact of the fixed-dimensional window. When
subsystem dimension scales with $N$ (fixed environment dimension),
the ratio of imaginary to real off-diagonal norms plateaus at
$\approx 0.27$. An analytical proof that this ratio remains bounded
away from zero as $N \to \infty$ would close this falsification
criterion.
...
(Remark~\ref{rem:toy_model}); a failure at larger $N$ or with
